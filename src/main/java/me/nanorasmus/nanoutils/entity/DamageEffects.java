...
public class DamageEffects implements Listener {
    ...
    private static final String FILE_PATH = "damage_effects";
    private static HashMap<UUID, HashMap<String, DamageEffect>> effects = new HashMap<>();

    /**
     * DO NOT INSTANTIATE UNLESS YOU ARE THE MAIN CLASS OF NANOLIB ITSELF
     */
    public DamageEffects(Main main) {
    ...
    public void DamageEvent(EntityDamageEvent e) {
        ...
        if (effects.containsKey(entityUUID)) {
            for (DamageEffect effect : effects.get(entityUUID).values()) {
                effect.run(e);
            }
        }
    }
    ...
    public static void AddEffect(Entity entity, String id, DamageEffect effect) {
        if (!HasEffect(entity)) {
            effects.put(entity.getUniqueId(), new HashMap<>());
        }

        effects.get(entity.getUniqueId()).put(id, effect);
    }
